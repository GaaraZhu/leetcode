func convert(s string, numRows int) string {
	...
	resultArray := make([]string, numRows)
	for i := 0; i < numRows; i++ {
		resultArray[i] = ""
	}
	goingDown := false
	currentRow := 0
	for i := 0; i < len(s); i++ {
		resultArray[currentRow] = resultArray[currentRow] + s[i:i+1]
		if i == 0 || i%(2*numRows-2) == 0 || ((i+1-numRows)%(2*numRows-2) == 0) {
			goingDown = !goingDown
		}
...
func originalConvert(s string, numRows int) string {
	if numRows == 1 {
		return s
	}
	length := len(s)
